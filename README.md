...
## Guest-боты: возможности и ограничения

### Что умеют guest-боты

- Подключаются отдельным токеном через меню владельца (`/start` → `Guest-боты` в основном боте).
- Работают как отдельный runtime-процесс, но используют общую БД проекта.
- Принимают персональные команды, созданные владельцем/DEV в ЛС guest-бота через `/start`.
- Для персональных команд можно задавать текст, медиа и inline-кнопки в том же формате, что и в `/settings` → `Модули` → `Команды`.
- Поддерживают вызов команд в нескольких форматах:
  - `@username_бота команда`
  - `/команда@username_бота`
  - `команда`
- Для каждой персональной команды можно задать доступ:
  - `Все пользователи`
  - `Для владельца`
- Если персональная команда не найдена, guest-бот может ответить через ИИ.
- Для ИИ есть отдельная настройка в интерфейсе guest-бота:
  - `ИИ для всех`
  - `ИИ для владельца`
- Модель ИИ выбирает только владелец guest-бота в ЛС гостевого бота текстом:
  - `groq`
  - `gemini google ai studio`
- ИИ автоматически собирает контекст из внешних источников (DuckDuckGo + Wikipedia RU/EN), извлекает текст страниц и добавляет список источников в ответ.
- Если источники не найдены, ИИ использует осторожный fallback-ответ без выдуманных деталей.

### Порог включения ИИ (минимум слов)

- Для обычных пользователей: от **4 слов**.
- Для владельца и DEV-пользователей: от **2 слов**.
- Порог применяется только когда текст не совпал с персональной командой.

### Что guest-боты НЕ умеют / ограничения

- Не управляются через общий интерфейс модерации как основной бот: у них отдельный runtime и отдельный PM-интерфейс команд.
- Не отвечают на пустые/нетекстовые сообщения как на AI-запрос.
- При сбое ИИ бот возвращает явную причину проблемы (например, неверный ключ, лимит, таймаут или недоступность модели) и пишет расширенную диагностику в лог.
- Качество и скорость AI-ответа зависят от доступности внешних источников и модели.
- Форматирование ответа ограничено безопасным HTML Telegram (часть тегов/разметки очищается).
- Длина AI-ответа и служебных блоков ограничена внутренними лимитами Telegram/runtime.
- Если guest-бот отключён в БД, runtime останавливается и не обслуживает команды.
- Без корректных переменных окружения (например, `GROQ_API_KEY` или `GEMINI_API_KEY` для ИИ) AI-функции недоступны.
